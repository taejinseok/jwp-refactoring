...
class TableServiceTest {
    ...
    @Test
    void changeEmpty() {
        OrderTable2 그루핑되지않는테이블 = orderTableRepository.save(new OrderTable2(10, false));
        Long 테이블식별자 = 그루핑되지않는테이블.getId();
        Order2 완료된주문 = new Order2(그루핑되지않는테이블, OrderStatus.COMPLETION,
    ...
    void changeEmpty_fail_if_table_contains_group_table() {
        TableGroup2 테이블그룹 = tableGroupRepository.save(
                new TableGroup2(LocalDateTime.of(2020, 10, 15, 23, 50)));
        OrderTable2 그루핑된테이블 = orderTableRepository.save(new OrderTable2(테이블그룹, 10, false));
    ...
    void changeEmpty_fail_if_table_s_order_is_not_completion() {
        OrderTable2 테이블 = orderTableRepository.save(new OrderTable2(10, false));
        Long 테이블의식별자 = 테이블.getId();
        Order2 테이블의_완료상태가아닌_주문 = orderRepository.save(
                new Order2(테이블, OrderStatus.COOKING, LocalDateTime.of(2020, 8, 20, 20, 20)));
    ...
    void changeNumberOfGuests_fail_if_number_of_guest_is_negative() {
        final OrderTable2 점유중인테이블 = orderTableRepository.save(new OrderTable2(5, false));
    ...
    void changeNumberOfGuests_fail_if_table_is_empty() {
        final OrderTable2 비어있는테이블 = orderTableRepository.save(new OrderTable2(0, true));
