...
class ProductRestControllerTest extends ControllerTest {
    ...
    void create() throws Exception {
        ProductCreateRequest 제품_추가_요청 = new ProductCreateRequest("치킨", BigDecimal.valueOf(16_000));

        String 제품추가_API_URL = "/api/products";
        ResultActions resultActions = create(제품추가_API_URL, 제품_추가_요청);

        resultActions
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.id", notNullValue(Long.class)))
                .andExpect(jsonPath("$.name", is("치킨")))
                .andExpect(jsonPath("$.price", is(16_000)));
